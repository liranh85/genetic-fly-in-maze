class Fly {
  ...
  _setTransitionDuration () {
    this.flyElm.style.transitionDuration = `${this.interval}${
      this.transitionDurationUnit
    }`
  }
      ...
      const autoPilotInterval = () => {
        const direction =
          directions && directions.length
            ? this._parseDirection(directions.shift())
  ...
  flyTo (direction) {
    ...
    if (this.reachedExit(coordinates)) {
      ...
      this.flyElm.style.transform = `translate(${this.coordinates.x}px, ${
        this.coordinates.y
      }px)`
      window.setTimeout(() => {
      ...
      this.world.isElementInWorld(coordinates, this.width, this.height) &&
      !this.world.isElementOnOtherElement(
        coordinates,
        this.width,
        this.height,
        'obstacle'
